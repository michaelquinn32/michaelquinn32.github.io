...
Our class simplifies our life in a couple ways. First, it takes advantage of some of the most common applications of [non-standard evaluation in R](http://adv-r.had.co.nz/Computing-on-the-language.html). We can pass the function either named values `geyser(a = 1, b = 2)` or a list of values when creating a new `geyser`. It has some defensive programming features, too. The generator throws an error if our list is nested, we have only one geyser, or if we have duplicate names or missing names.
