...
Passing either named values `geyser(a = 1, b = 2)` works fine, as does a list of values. The generator throws an error if our list is nested, we have only one geyser, or if we have duplicate names or missing names.
