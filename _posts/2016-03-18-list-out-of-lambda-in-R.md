...
Take a second to mull these over. And all you pedants over there, cool it with the "you're abusing the class system nonsense." I know I'm abusing the class system, but if we're going to create "objects" out of lists and preserve even a modicum of beauty in our code, we're going to need to take some short cuts. If we can all take it easy for just one second, I promise that everyone will have a lot of fun.
...
Alright, let's get back on track. What have we done? Believe it or not, we've got almost everything we need (save one function) for Losh's implementation of a list: "a function that returns its head or tail." While R doesn't use inheritance like C++, Java or Python, it's still useful to this of the preceding functions as a "base class" that we'll build off of later.
...
While the call to `structure` make everything a little cluttered, the heart of prepend is the same style closure that we saw in `empty_list`. But now, the anonymous function replaces the closure's arguments with the prepended element, the list and `FALSE` (a prepended list is no longer empty!).
...
We're now counting recursively. Empty lists have no length. They're our base case. Otherwise, we call the function again on the tail of the list and add one. This strips off one element from the list. Each operation strips one and adds one. And just like that, we have the list's length.
...
I don't want to get to deep into R's non-standard evaluation and meta-computing tools. Hadley Wickham explains these well. Instead, let's just implement a version of `rerun` where we require the user to specify that they have an expression in advance.
...
* **Reductions**: take a list a recursively fold it down to a scalar
...
We wrap up our little tour of functional programming by looking at reductions. Generally speaking, this class of functions take a binary function and *folds* it along a list, eventually resulting in a single scalar. What does that mean? Consider the following case, reducing a sequence of numbers with addition.
...
This is another obvious candidate for recursion, but a better implementation keeps track of the intermediate calculations through a value in the arguments. Fortunately, most reductions allow for initialization with a specific value, and we can use that. With that, we can implement two different reductions, one for each "direction" of the list.
...
`reduce_right` is a bit more complicated to use, as it leads to many counterintuitive results. It's usually helpful to work out the problem on paper before going that route.
